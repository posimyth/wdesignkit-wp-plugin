...
const Save_template = (props) => {
    ...
    const OnSaveTemplate = async (query) => {
        ...
        let form_arr = {
            ...
            'post_id': newFilterArgs?.post ? newFilterArgs.post : ''
        }
    ...
    const clickData = async (save_type) => {
        ...
        if ((save_type == 'new_temp') && !temp_name) {
            ...
            props.wdkit_set_toast([__('Enter Template Name', 'wdesignkit'), __('Title can not be empty!', 'wdesignkit'), '', 'danger']);
            return;
        }
        ...
        if (!content || !data) {
            props.wdkit_set_toast([__('Page Content is empty', 'wdesignkit'), __('Page should have some content', 'wdesignkit'), '', 'danger'])
            return;
        }
        ...
        if ('new_temp' == save_type) {
            ...
            let temp_title = selectedTemplate?.title ? selectedTemplate.title : '';

            let new_data = Object.assign({}, data, { 'title': temp_title });
            var result = await ReplaceTemp(JSON.stringify(new_data));
            return;
        }
        ...
        return (
            <div className="wkit-model-transp wkit-sucessfully-save-to-cloud wkit-popup-show">
                <div className='wkit-plugin-model-content'>
                    ...
                    <div className="popup-header">{__('Upload Template', 'wdesignkit')}</div>
                    <div className='wkit-successfully-imported'>
                        ...
                        <div className="wkit-text-center">
                            <div className="wkit-success-heading wkit-get-success">{__('Successfully Uploaded', 'wdesignkit')}</div>
                            <div className="wkit-desc">{__('To Edit fully visit WdesignKit admin and make it live.', 'wdesignkit')}</div>
                        </div>
                        {save_id &&
                            <button type='submit' className='wkit-save-btn wkit-btn-class'>
                                <a href={save_editurl} target="_blank" rel="noopener noreferrer">{__('Edit Fully', 'wdesignkit')}</a>
                            </button>
                        }
                    </div>
                </div>
    ...
    const Plugin_selection = () => {
        if (userData?.plugin?.length > 0) {
            return (
                <Fragment>
                    <label className={"wkit-label-heading"}>{__('Plugins Used', 'wdesignkit')}</label>
            ...
            if (userData?.plugin?.length == 0) {
                return <div className={'wkit-no-template-data'}>{__('No Plugin List', 'wdesignkit')}</div>
            } else {
    ...
    const Select_save_template = () => {
        return (
            <div className='wkit-switch-setting-wrap'>
                <div className='wdkit-switch-text' style={{ color: isChecked ? '#19191B' : '#C22076' }}>{__('New Template', 'wdesignkit')}</div>
                ...
                <div className='wdkit-switch-text' style={{ color: isChecked ? '#C22076' : '#19191B' }}>{__('Existing Template', 'wdesignkit')}</div>
            </div>
        )
    }
    ...
    const SelectRpTemp = (temp_id) => {
        if (temp_id && props?.wdkit_meta?.template?.length > 0) {
            ...
            if (index > -1) {
                ...
                setselectedTemplate('');
                props.wdkit_set_toast([__('Template Not Found', 'wdesignkit'), __('selected Template is Not Found', 'wdesignkit'), '', 'danger'])
            }
        }
    }
            ...
            <Fragment>
                <div className={"wkit-existing-template"}>
                    <div className='wkit-search-template'>
                        <label className={"wkit-label-heading"}>{__("Search Template", 'wdesignkit')}</label>
                        <div className='wkit-search-template-input' >
                            <input type="text" placeholder={__('Search Template', 'wdesignkit')} className='wkit-existing-temp-search' value={find_template} onChange={(e) => setfind_template(e.target.value)} />
                            {tempLoader ?
                            ...
                            <Fragment>
                                <div className='wkit-find-temp-list-content'>
                                    <ul className="wkit-find-temp-list">
                                        <span className='wkit-Flist-notice'>{searchTemp ? `Search "${searchTemp}"` : __('Recently added', 'wdesignkit')}</span>
                                        {tempLoader == false ?
                                            <Fragment>
                                                {FindTemplateList?.length > 0 ?
                                                    ...
                                                    <div className='wkit-no-data'>
                                                        ...
                                                        <span>{__('Search Template can not found', 'wdesignkit')}</span>
                                                    </div>
                                                }
                                            </Fragment>
                                            :
                    ...
                    <div className='wkit-select-template'>
                        {selectedTemplate ?
                            <div className='wkit-selected-template-content'>
                                <span className='wkit-selected-template-info'>{__('Do you want to replace this?', 'wdesignkit')}</span>
                                <div className='wkit-selected-template'>{Showtemp()}</div>
                                <div className="wkit-quickedit-note">
                                    <span>
                                        <b>{__('Note : ', 'wdesignkit')}</b>{__('Replacing the template will erase the current design. This action cannot be undone, and the old design will be permanently lost.', 'wdesignkit')}
                                    </span>
                                </div>
                            </div>
                            :
                ...
                {selectedTemplate &&
                    <div className="wkit-replace-button">
                        <button className="wkit-btn-class" onClick={() => { setconfirmPopup(true) }}>{__('Replace', 'wdesignkit')}</button>
                    </div>
                }
            </Fragment>
        )
                ...
                <Fragment>
                    <div className="wkit-select-main">
                        ...
                        <div className='wkit-save-temp-content' style={isChecked ? { display: 'none' } : {}}>
                            <div className={"wkit-save-heading"}>{__('Save Template', 'wdesignkit')}</div>
                            <div className={"wkit-select-template"}>
                                <div className={"wkit-col wkit-flex-wrap-none wkit-col-gap"}>
                                    <div className='kit-field-wrap'>
                                        <label className={"wkit-label-heading"}>{__("Template Name", 'wdesignkit')}</label>
                                        <input type={"text"} placeholder={__('Enter Template Name', 'wdesignkit')}
                                    ...
                                    <div className='kit-field-wrap'>
                                        <label className={"wkit-label-heading"}>{__("Select Workspace", 'wdesignkit')}</label>
                                        <div className='wkit-saveTemplate-dowpDown'>
                                            <select className={"wkit-select " + errorSelectMsg} value={select_type} onChange={(ev) => setSelectType(ev.target.value)}>
                                                <option value={'my_upload'}>{__('My Upload', 'wdesignkit')}</option>
                                                <optgroup label={__("My Workspace", 'wdesignkit')}>
                                                    {selectWork &&
                                ...
                                <div className={"wkit-col wkit-flex-wrap-none wkit-col-gap"}>
                                    <div className='kit-field-wrap'>
                                        <label className={"wkit-label-heading"}>{__("Template Type", 'wdesignkit')}</label>
                                        <div className='wkit-saveTemplate-dowpDown'>
                                            <select className={"wkit-select " + errorSelectMsg} value={temp_type} onChange={(ev) => { setTempType(ev.target.value), setCategory('') }}>
                                                <option value={'pagetemplate'}>{__('Page Template', 'wdesignkit')}</option>
                                                <option value={'section'}>{__('Section', 'wdesignkit')}</option>
                                            </select>
                                            <svg width="12" height="10" viewBox="0 0 12 8" fill="#878787" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M9.87998 1.28957L5.99998 5.16957L2.11998 1.28957C1.72998 0.89957 1.09998 0.89957 0.70998 1.28957C0.31998 1.67957 0.31998 2.30957 0.70998 2.69957L5.29998 7.28957C5.68998 7.67957 6.31998 7.67957 6.70998 7.28957L11.3 2.69957C11.69 2.30957 11.69 1.67957 11.3 1.28957C10.91 0.90957 10.27 0.89957 9.87998 1.28957Z" />
                                            </svg>
                                    ...
                                    <div className='kit-field-wrap'>
                                        <label className={"wkit-label-heading"}>{__("Category", 'wdesignkit')}</label>
                                        <div className='wkit-saveTemplate-dowpDown'>
                                            <select className={"wkit-select"} value={category} onChange={(ev) => setCategory(ev.target.value)}>
                                                <option value={''}>{__('Select Category', 'wdesignkit')}</option>
                                ...
                                {isSaving ?
                                    ...
                                    <div className={!temp_name.trim() ? 'wkit-disable-btn-class' : ''}>
                                        <button type="button" className={`wkit-save-btn wkit-btn-class`}
                                            ...
                                            onClick={() => clickData('new_temp')}>{__('Save Template', 'wdesignkit')}</button>
                                    </div>
                                }
                            </div>
                            {successMsg &&
