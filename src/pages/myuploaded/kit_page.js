    ...
    const SelectAllHTML = () => {
        return (
            <div className='wkit-kit-selectAll-btn'>
                ...
                <label htmlFor='wkit-selectAll-kitTemp'>Select All</label>
            </div>
        );
    };
        ...
        <div className="wkit-myupload-main">
            <div className='wkit-kit-header'>
                <div className='wkit-kit-header-back'>
                    <Link to={`/${GetFirstpathName()}`} state={yourQueryParam}>
                ...
                <Page_header
                    ...
                    svg={<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 36 36" fill="none" style={{ width: '28px' }}>
            ...
            {loading == false && kitData?.data?.template?.length > 0 && (downloadMulti?.pages?.length > 0 || downloadMulti?.sections?.length > 0) &&
                <div className='wkit-total-template-wrapper'>
                    <div className='wkit-total-title'>{'Total Template Selected ( ' + (downloadMulti?.pages?.length + downloadMulti?.sections?.length) + '/' + (kitData?.data?.template?.length ? kitData?.data?.template?.length : 0) + ' )'}</div>
                    <div className='wkit-button-wrapper-group'>
                        <button type='submit' className='wkit-download-all' onClick={() => { DownloadTemp() }}>{__('Download')}</button>
                    </div>
                </div>
            }
            {downTempId &&
